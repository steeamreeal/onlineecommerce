...
- **Cadastro do domínio na Vercel é automático** (`lib/vercel-domains.ts`, Vercel Domains API): quando o lojista salva `Loja.dominioProprio` em `loja.atualizarDominioProprio`, o backend chama `POST /v10/projects/{id}/domains` para registrar o domínio no projeto da plataforma — não é mais preciso o admin entrar manualmente em Settings → Domains a cada loja nova. Trocar/remover o domínio também remove o antigo da Vercel (`DELETE /v9/projects/{id}/domains/{domain}`, melhor esforço). O DNS continua sendo responsabilidade do lojista (isso não dá pra automatizar, é do lado do provedor dele) — a tela (`components/dashboard/dominio-proprio-form.tsx`) segue mostrando a instrução de CNAME/ALIAS, agora com um selo de status ("Ativo"/"Aguardando DNS") consultado via `loja.statusDominioProprio` (`GET /v6/domains/{domain}/config`, campo `misconfigured`).
  - Credenciais (variáveis de ambiente, nunca commitadas): `VERCEL_API_TOKEN` (token com permissão no projeto), `VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID` (só se o projeto pertence a um time). **Opcional**: sem essas variáveis configuradas, o fluxo cai de volta pro cadastro manual antigo (`VercelDomainsIndisponivelError` é tratado como no-op silencioso nas duas mutations/query) — nenhuma instalação quebra por falta delas, só perde a automação.
  - Domínio raiz/apex não aceita `CNAME` — a Vercel aceita via `ALIAS`/`ANAME` dependendo do provedor de DNS do lojista (fora do nosso controle); o formulário já avisa disso quando detecta domínio sem subdomínio.
  - Se a Vercel mudar a política/formato dessa API, revalidar a documentação oficial (`vercel.com/docs/rest-api/projects/*domain*`) antes de alterar `lib/vercel-domains.ts`.
